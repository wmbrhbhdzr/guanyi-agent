...
| Python               | 3.13.0（全局 `C:\Python\python.exe`；项目依赖仅装入虚拟环境 `.venv\Scripts\python.exe`） |
...
2. **版本兼容性检查**：Python 3.13 发布时间较新，部分第三方库可能尚未提供对应的预编译 wheel。本次安装中全部依赖**均成功安装，未出现因缺少预编译包而失败的情况**；安装过程中实际遇到的障碍是依赖文件的编码解析问题，见 3.2 节问题 6。
...
* **问题 11**：在**未激活虚拟环境**的终端中执行 `python scripts\api_smoke_test.py`，接口调用、模型回答、token 统计与费用计算**全部正常**，但脚本**静默改用自制测试图**——输入图片由 1280×800 的真实截图变为 640×360 的合成图，token 用量随之由 711 降到 261，**若不细看极易误认为运行正常**。
  * **分析**：该终端中的 `python` 指向全局解释器 `C:\Python\python.exe`，而 `mss` 与 `pyautogui` **只安装在 `.venv` 中**；同时 `requests` 与 `Pillow` 全局也已安装，因此接口调用与图像生成仍然成功，**只有屏幕采集失败**。脚本当时对采集失败做了静默回退（`capture_screen(...) or make_test_image(...)`），未给出任何提示。
  * **本质**：这属于**错误被部分依赖掩盖的静默降级**——依赖缺失并未导致整体失败，而是按模块逐个暴露：缺的模块所对应的功能悄悄失效，其余功能照常运行，因此从表面输出上看不出异常。
  * **处理**：① 运行前先激活虚拟环境（`.\.venv\Scripts\Activate.ps1`），或直接使用 `.\.venv\Scripts\python.exe`；② 修改脚本，使采集失败时**不再静默回退**，改为输出可自诊断的告警（含当前解释器路径、是否处于虚拟环境、失败原因与处理方法）；③ 在输出中新增"图片来源"一行（实时截屏／自制测试图／命令行指定文件），并写入运行记录；④ 支持显式指定图片路径以绕开自动采集。
  * **验证**：① 分别对全局解释器与虚拟环境解释器导入 `requests`、`PIL`、`mss`、`pyautogui` 四个模块，确认全局解释器缺少后两者、虚拟环境四者均可用；② 用全局解释器复跑脚本，新告警正确输出解释器路径、虚拟环境状态与原因说明；改用虚拟环境解释器并显式指定图片路径后，`图片来源` 正确显示为"命令行指定文件"，运行正常。
...
| 最新 Commit Hash（报告定稿时） | `83fc6be13b3cdc14a1457d40733834809a8b037a`      |
| 开源许可                   | MIT License（`LICENSE`，已随首个提交入库）                |

> 提交作者使用的邮箱为 GitHub 提供的匿名邮箱，避免真实邮箱在公开仓库中暴露。

**提交历史（截至报告定稿共 10 次；本报告最终的少量修订随后续提交入库）**
...
| `2652174` | fix: 冒烟测试截屏失败时给出可自诊断告警（解释器与虚拟环境状态）       | 24 行增，5 行删      |
| `ac5a904` | docs: 报告改用多次实测区间，更新图注并清理未引用截图                 | 4 文件，62 增 61 删  |
| `83fc6be` | docs: 补充问题11与三次读数对比，图2换用多次运行记录                   | 3 文件，26 增 16 删  |
...
83fc6be13b3cdc14a1457d40733834809a8b037a    HEAD
83fc6be13b3cdc14a1457d40733834809a8b037a    refs/heads/main
```
...
| **端到端时延**   | **多次实测区间：1942 – 8321 ms（随截图内容与回答长度变化）**                                                  |
| **用量**         | 输入 261–935 token（其中一次缓存命中 127 token）；输出 307–1500 token（含模型内部推理 token）                        |
| **结束原因**     | 多数运行以 `stop` 正常结束；**其中一次因输出达到 `max_tokens` 上限而返回 `length`（被截断）**——说明对于"详细描述画面"这类开放式任务，上限设为 1500 仍可能不足。故后续智能体应采用**结构化短输出**，并显式校验结束原因与返回内容 |
| **费用（折算）** | 按空闲单价折算 **0.0014 – 0.0067 元**；同一用量下高峰单价为空闲单价的两倍（计算过程见下）                          |

**读数准确率的一次实测观察**

在其中一次使用**真实桌面截图（1280×800，画面含 IDE 与聊天窗口）**作为输入的测试中，模型对图中文字的基本识别是正确的，但出现了两处误读：
...
而使用**自制的高对比度合成图**测试时（图中仅含 `HP 68/80`、`ENERGY 3/3`、卡牌名称与数值等大号文字），模型 7 处文字**全部读对**；另一次以**终端窗口截图**（1263×1113，等宽字体、文本稀疏）为输入时，模型将整屏终端内容（含命令、中文提示、数字与表格）**完整准确读出，未出现误读**。

**三次测试的对比说明**：读数准确率并不取决于"能否识别文字"，而取决于**文本密度与字号**——大号、稀疏、高对比度的文本（合成图、终端窗口）可以完整读对，而信息密集、字号较小、存在多窗口干扰的真实界面会出现误读。因此后续开发中**不能假设"模型读屏必然正确"**，需要专门设计读数准确率的评测方法，并在决策流程中加入必要的校验环节。该观察已作为后续选题阶段的重要输入。
...
图 2　视觉 API 冒烟测试的一次运行输出（2026-09-12 01:20:51，空闲时段；输入为**实时截屏**的真实桌面画面）：依次显示请求时间与时段判定、模型调用名、接口地址、请求类型、输入图片规格与来源、返回结果、结束原因、端到端时延、token 用量与费用折算。模型正确读出画面中的窗口、标签页、任务栏与代码编辑器等内容，并主动列出"看不清或无法确认的部分"，对应提示词中"看不清的部分请明确说明、不要猜测"的要求。

![冒烟测试运行输出](image/实验1-AI软件开发环境安装与配置/1789147412965.png)

> 注：本次输入为实时截屏，而被采集的桌面画面中恰好包含终端窗口的历次运行输出，因此模型回答里出现了对前几次运行日志的转述——**图中出现的多段日志来源于"被截图的那块屏幕"，并非本次运行重复执行**。此外，此前有一次运行因在未激活虚拟环境的解释器中执行而截屏失败、回退为自制测试图（640×360），该问题记为问题 11；脚本现已在该情况下输出明确告警，不再静默回退。
...
| Git 可提交与推送   | ✅ 通过 | 共 10 次提交，远端`refs/heads/main` 的 Commit Hash 与本地 `HEAD` 一致（`83fc6be`）                               |
...
| AI 协作记录可查看  | ✅ 通过 | `docs/AI协作记录.md`（含工具对比表与 11 条配置问题记录）                                             |
...
* 完成 Git 与 GitHub 配置：仓库已创建并设置 MIT 许可与 `.gitignore`，完成 10 次提交与推送，首个 Commit Hash 为 `f14dfadb16869388c7f78652e2e36bdcaba44b32`。
...
本次实验共记录 11 个实际问题（编号与 `docs/AI协作记录.md` 一致），其中最具代表性的是**问题 4（控制台代码页与 Git 输出编码冲突）**：
...
**问题 11（未激活虚拟环境导致静默降级）** 则展示了另一类更隐蔽的失败：依赖缺失并未让程序整体失败，而是**按模块逐个暴露**——`requests` 与 `Pillow` 全局可用，故接口调用与图像生成照常成功；只有 `mss` 与 `pyautogui` 缺失，导致屏幕采集悄悄失效并回退为合成图。**这类"部分功能静默降级"比直接报错更危险，因为它的输出看起来完全正常。** 处理方式是把静默回退改为可自诊断的告警，并在输出中显式标注数据来源，使异常状态无法被忽略。问题 10 与问题 11 合起来指向同一条经验：**程序不能只报告"是否抛异常"，还要报告"结果是否有效、数据来自哪里"。**
...
**结论**：本次实验完成了从环境搭建到可运行、可复现、可核验的完整闭环。过程中暴露的 11 个问题均已在报告中记录原因、处理方法与验证方式，其中的编码类问题（问题 4、6）具有共性，已通过写入长期配置的方式一次性解决，后续开发不会重复遇到；其余问题所反映的"**工具输出需要被验证**"这一经验，也已转化为脚本中的显式检查项。
