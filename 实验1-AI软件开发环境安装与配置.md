...
| 最新 Commit Hash（报告定稿时） | `ac5a9041a64a8cd1e55e1fa78dce280eda60057e`      |
| 开源许可                   | MIT License（`LICENSE`，已随首个提交入库）                |

> 提交作者使用的邮箱为 GitHub 提供的匿名邮箱，避免真实邮箱在公开仓库中暴露。

**提交历史（截至本报告定稿共 9 次；本报告自身的改动随后续提交入库）**
...
ac5a9041a64a8cd1e55e1fa78dce280eda60057e    HEAD
ac5a9041a64a8cd1e55e1fa78dce280eda60057e    refs/heads/main
```
...
图 2　视觉 API 冒烟测试的运行记录（连续三次）：依次显示请求时间与时段判定、模型调用名、接口地址、请求类型、输入图片规格与来源、返回结果、结束原因、端到端时延、token 用量与费用折算。其中一次运行的回答中，模型明确列出了"不确定或无法确认的部分"，对应提示词中"看不清的部分请明确说明、不要猜测"的要求。

![多次运行记录](image/实验1-AI软件开发环境安装与配置/1789147412965.png)

> 注：图中第一次运行的输入为**自制测试图（640×360）**——当时在未激活虚拟环境的终端中运行，屏幕采集不可用而回退为合成图（见问题 11）；其后两次为**真实屏幕截图（1280×800）**。上表给出的是多次运行的实测区间，故无需与任一单次运行逐项对应。
...
| Git 可提交与推送   | ✅ 通过 | 共 9 次提交，远端`refs/heads/main` 的 Commit Hash 与本地 `HEAD` 一致（`ac5a904`）                               |
...
* 完成 Git 与 GitHub 配置：仓库已创建并设置 MIT 许可与 `.gitignore`，完成 9 次提交与推送，首个 Commit Hash 为 `f14dfadb16869388c7f78652e2e36bdcaba44b32`。
