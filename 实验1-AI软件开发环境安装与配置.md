...
| 最新 Commit Hash           | `b773a5ee72fa8599debf4277e5c182cde5a2cdb6`                                                 |
...
| `b773a5e` | fix: 为 requirements.txt 添加编码声明，修复 cp936 环境下 pip 解析失败 | 1 行新增             |
...
b773a5ee72fa8599debf4277e5c182cde5a2cdb6    HEAD
b773a5ee72fa8599debf4277e5c182cde5a2cdb6    refs/heads/main
```
...
| 输入                 | 屏幕截图 1280×800，PNG 格式，约 448–768 KB                                                                                     |
| 返回结果             | 基本正确识别图片内容，未出现编造；但在信息密集的小字区域出现少量误读（详见下方说明）                                                                     |
| **端到端时延** | **多次实测：3174 ms / 3248 ms / 3318 ms / 3456 ms；修正 `max_tokens` 后完整回答为 5323 ms**                              |
| **用量**       | 输入 711 token（缓存命中 0 / 未命中 711）；输出 916 token（含模型内部推理 token）；合计 1627 token                          |
| **结束原因**   | `stop`（正常结束，输出未被截断）                                                                                             |
| **费用（折算）** | 本次运行处于**空闲时段**，按空闲单价折算 **0.004375 元**（若按高峰单价折算为 0.008750 元，计算过程见下）                     |

**读数准确率的一次实测观察**

本次测试所使用的输入为一张真实桌面截图（VS Code 与聊天客户端窗口），模型对其中文字的基本识别是正确的，但出现了两处误读：
...
本次运行时间为 2026-09-12（周六）01:00，**处于空闲时段**，故按空闲单价折算：
...
**五次运行全部处于空闲时段**，与"深夜与周末均不在高峰窗口内"的预期一致。该问题已记入 `docs/AI协作记录.md`（问题 9），脚本已改为按官方定义判断（仅工作日 09:00–12:00、14:00–18:00 计为高峰），并在输出中增加星期与时段显示、以及"折算值以服务端账单为准"的提示。**实际扣费可通过模型平台的用量与账单页面核对。**
...
![视觉 API 冒烟测试输出](image/实验1-AI软件开发环境安装与配置/1789143042193.png)

> 注：该截图为脚本修正前的输出，其中"高峰时段"标注对应时段判定修正前的旧规则，`max_tokens` 亦为修正前的取值 400（相关说明见问题 9、问题 10）。修正后重新运行的完整输出见本节文字及 `runs/` 目录下的记录。
...
| Git 可提交与推送   | ✅ 通过 | 共 5 次提交，远端`refs/heads/main` 的 Commit Hash 与本地 `HEAD` 一致                       |
...
* 完成 Git 与 GitHub 配置：仓库已创建并设置 MIT 许可与 `.gitignore`，完成 5 次提交与推送，首个 Commit Hash 为 `f14dfadb16869388c7f78652e2e36bdcaba44b32`。
