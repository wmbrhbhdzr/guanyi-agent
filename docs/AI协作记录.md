...
| 11 | 在**未激活虚拟环境**的终端中执行 `python scripts\api_smoke_test.py`：接口调用、模型回答、token 统计与费用计算**全部正常**，但脚本**静默改用自制测试图**（输入由 1280×800 真实截图变为 640×360 合成图），token 用量随之从 711 降到 261，若不细看极易误认为运行正常 | 该终端中的 `python` 指向全局解释器 `C:\Python\python.exe`，而 `mss` 与 `pyautogui` **只安装在 `.venv` 中**；同时 `requests` 与 `Pillow` 全局也已安装，因此接口调用与图像生成仍能成功，**只有屏幕采集失败**。脚本当时对采集失败做了静默回退（`capture_screen(...) or make_test_image(...)`），没有任何提示——属于**错误被部分依赖掩盖的静默降级** | ① 运行前先激活虚拟环境 `.\.venv\Scripts\Activate.ps1`，或直接使用 `.\.venv\Scripts\python.exe`；② 修改脚本：采集失败时不再静默回退，改为输出**可自诊断的告警**（含当前解释器路径、是否虚拟环境、失败原因与处理方法）；③ 在输出中新增"图片来源"一行（实时截屏／自制测试图／命令行指定文件），并写入运行记录 JSON；④ 支持显式指定图片路径以绕开自动采集 | 分两步验证：① 对全局解释器与虚拟环境解释器分别导入四个模块，确认全局解释器缺 `mss`、`pyautogui`，虚拟环境四个模块均可用；② 用全局解释器复跑脚本，新告警正确输出解释器路径、虚拟环境状态与原因说明。改用虚拟环境解释器并显式指定图片路径后，`图片来源` 正确显示为"命令行指定文件"，运行正常 → 通过 |
